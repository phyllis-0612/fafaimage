...
1. 点击右下角悬浮球打开面板
2. 在「API 配置」填入你的第二 API 地址、密钥和模型名
3. 在「系统提示」写给提取模型的角色设定
4. 在「基础模板」粘贴完整的 `image###...{Description}...###` 模板，用 `{Description}` 标记插入位置
5. 在「角色锚点」填入当前卡片角色的外貌描述
6. 在「提取规则」填入 Description 的撰写规范
7. 开始对话，新 AI 消息到达后自动提取并显示在预览区
8. 确认或编辑后点击「确认注入」
